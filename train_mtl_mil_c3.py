...
class MultiTaskResNet50(nn.Module):
    def __init__(self, task1_classes=3, task3_classes=4, task4_classes=4):
    ...
    def forward(self, x):
        ...
        return task1_output, task3_output, task4_output
...
def cal_loss(loss1, loss3, loss4):
    # 计算动态权重
    losses = torch.tensor([loss1.item(), loss3.item(), loss4.item()])
    weights = 1 / (losses + 1e-6)  # 加小常数避免除零
    normalized_weights = weights / weights.sum()

    # 加权损失
    loss = normalized_weights[0] * loss1 + normalized_weights[1] * loss3 + normalized_weights[2] * loss4
...
def train_model(model, train_loader, valid_loader, optimizer, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for bags, l1, l2, l3, l4 in tqdm(train_loader):
            ...
            loss = cal_loss(loss1, loss3, loss4)
            loss.backward()
            optimizer.step()
        ...
        if (epoch+1)%5 == 0:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            # 保存模型
            torch.save(best_model_wts, f'results/mil_mtl_se_epoch{epoch}_V0.5_c3.pth')
...
# 开始训练
best_wts, history = train_model(model, train_loader, valid_loader, optimizer, num_epochs=10)
from datetime import date
# 保存history为CSV文件
with open(f'history_mil_mtl_{date.today()}_c3.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Epoch', 'Train Loss', 'Val Acc1', 'Val Acc3', 'Val Acc4', 'Val loss', 'Val acc'])
    for i, (train_loss, val_acc1, val_acc3, val_acc4, val_loss, val_acc) in enumerate(
            zip(history['train_loss'], history['val_acc1'], history['val_acc3'], history['val_acc4'], history['val_loss'], history['val_acc'])):
        writer.writerow([i+1, train_loss, val_acc1, val_acc3, val_acc4, val_loss, val_acc ])
